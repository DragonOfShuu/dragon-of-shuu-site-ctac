...
const useNavBarInit = () => {
    const [navBarData, setNavBarData] = useReducer(NavBarDataReducer, {
        isVisible: true,
        hiddenReasons: [],
        forceVisible: false,
    });
...
const NavBarDataReducer = (
    prevState: NavBarContextDataType,
    action: NavBarContextActionType,
): NavBarContextDataType => {
    switch (action.type) {
        case "visibility": {
            const newState = { ...prevState };

            if (action.visible) {
                newState.hiddenReasons = newState.hiddenReasons.filter(
                    (str) => str !== action.reasonKey,
                );
            } else {
                // Anti-pattern. Will prob remove later
                // [nested if statement]
                if (!newState.hiddenReasons.includes(action.reasonKey)) {
                    newState.hiddenReasons = [
                        ...newState.hiddenReasons,
                        action.reasonKey,
                    ];
                }
            ...
            return {
                ...newState,
                isVisible: newState.hiddenReasons.length === 0,
            };
        }
