...
const useProjectSearch = (initialProjects: ProjectType[]) => {
    ...
    const [projectBuffer, setProjectBuffer] = useState<ProjectType[]>(initialProjects)
    ...
    const scheduleSearch = (newSearchText?: string, newSearchTags?: string[]) => {
        if (startSearchDelay.current !== null)
            clearTimeout(startSearchDelay.current)

        startSearchDelay.current = setTimeout(() => {
            startTransition(async () => {
                const newProjects = await searchProjects(newSearchText ?? searchText, newSearchTags ?? searchTags)
                startTransition(() => {
    ...
    return {isPending, projectBuffer, allTags, searchText, setSearchText, searchTags, setSearchTags}
}
