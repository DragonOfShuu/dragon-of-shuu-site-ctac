...
export class BlockNode {
    ...
    constructor(
        coords: Coord,
        parentAndEnd:
            | { parent: BlockNode; end: BlockNode; trudgeValue: number }
            | undefined = undefined,
    ) {
        this.coords = coords;
        ...
        let normalize = globSettings.normalizeDirection
            ? parentAndEnd.trudgeValue
            : 1;

        this.g = this.parent.g + parentAndEnd.trudgeValue;
        this.h =
            Math.sqrt(
                (this.coords.y - this.goalNode.coords.y) ** 2 +
                    (this.coords.x - this.goalNode.coords.x) ** 2,
            ) *
            globSettings.heuristicMultiplier *
            normalize;
    ...
    public discoverChildren(
        ...
        createNeighborList(canDiagonal).forEach((nRelative) => {
            ...
            let newNode = new BlockNode(position, {
                parent: this,
                end: goalNode,
                trudgeValue: getTrudgeCost(displaySelf.trudge) * diagonalCost,
            });
...
const findFastestPath = (
    ...
    let children = currentNode.discoverChildren(
        grid,
        state.end_node,
        settings.canPerformDiagonals,
    );

    children.forEach((child) => {
        // Skip child if it is closed
        if (state.closed_list.find((n) => child.equals(n)) != undefined) return;
