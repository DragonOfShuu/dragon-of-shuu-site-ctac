...
const InnerVisualizer = (props: Props) => {
    ...
    return (
        <div className={`flex flex-col flex-grow gap-2`}>
            <div
                className={`flex flex-row items-stretch gap-4 self-center h-14 w-full px-4 ${toolbarEnabled ? "" : "opacity-50 pointer-events-none"}`}
            >
                {props.children}
            </div>
            ...
            <div
                ...
                style={{
                    ...
                    gridTemplateColumns: listOfSimilars(
                        `${(1 / vData.blocks[0]?.length) * 100}%`,
                        vData.blocks[0]?.length ?? 0,
                    ).join(" "),
                    gridTemplateRows: listOfSimilars(
                        `${(1 / vData.blocks.length) * 100}%`,
                        vData.blocks.length,
                    ).join(" "),
                }}
            >
...
const Visualizer = (props: { children: ReactNode }) => {
    return (
        <VisualizerContextComp>
            <InnerVisualizer>{props.children}</InnerVisualizer>
        </VisualizerContextComp>
    );
};
