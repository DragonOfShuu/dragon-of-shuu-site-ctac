...
type BlockType = Block & {blockClicked: (block: {x: number, y: number}, isNewClick: boolean) => void}
...
export default function DisplayBlock({
    ...
    const onMouseDown = (event: MouseEvent<HTMLDivElement, globalThis.MouseEvent>, newClick?: boolean) => {
    ...
    const onMouseOver = (event: MouseEvent<HTMLDivElement, globalThis.MouseEvent>) => {
        if (event.buttons!=0) {
