...
const ChooseSize = () => {
    ...
    return (
        <>
            <input 
                type="number" 
                onChange={(event) => setTempX(Number.parseInt(event.target.value)??'')} 
            ...
            <input 
                ...
                min={1}
                max={limit} 
                title="Y length of the board" />{/* className={`w-14`} */}
            <ToolBarButton onClick={rotateNumbers} alt={`Rotate Values`} image={rotateIcon} />
            
            <div className={`grow basis-full`} />

            <HowSizer />
            <ToolBarButton onClick={()=> setPage('PAINTER')} text="Finish" className={`px-8`} />
        </>
    )
}
