...
const TileBlock = ({selected, clickCallback, color, index, tooltip}: TileBlockProps) => {
    let dialogRef = useRef<HTMLDialogElement>(null)

    const clicked = () => { if (!selected) clickCallback(index); };
    ...
    return (
        <div className={`h-full w-12 flex items-stretch relative`}>
            <dialog ref={dialogRef} className={`bg-transparent absolute pointer-events-none`}>
            ...
            <div 
                ...
                <div className={`w-12 h-full`} style={{backgroundColor: color}} />
            </div>
        </div>
    )
}
...
export default function MultiChooser<TReturnType>({content, changedCallback}: ChooserProp<TReturnType>) {
        ...
        <div className="flex items-stretch justify-between" >
            {
                content.map((element, index)=> (
                    <TileBlock 
                        ...
                        index={index} key={index} selected={selected==index}
                        tooltip={element.tooltip!=undefined?element.tooltip:''} />
                ))
            }
        </div>
    )
