...
const Runner = ({}: Props) => {
    ...
    const previousState = useRef<Block[][]>( JSON.parse(JSON.stringify(vData.blocks)) );
    ...
    const triggerEnd = useCallback((path: {x: number, y: number}[]) => {
        ...
        path.forEach((block, index)=> {
            ...
            }, (sData.pathRevealSpeed/path.length) * index, index+1, path.length);
        })
    ...
    const SimHeart = (starState: StarState, clearCallback: Function) => {
        ...
        if (starState.recent_node!=undefined)
            setOperationText(`Finding... (Current expense: ${Math.round(starState.recent_node.f*100)/100})`)
    ...
    useEffect(()=> {
        ...
        leInterval = setInterval(SimHeart, sData.algoSpeed, state, clearCallback)
